...
export const handlepost = async (
  ...
  const token = multerReq.cookies?.token;
  if (!token) {
    res.status(401).json({ success: false, message: "Unauthorized" });
    return;
  }
  ...
  try {
    ...
    postDoc.postId = postDoc._id.toString();
    ...
    if (alreadyLiked) {
      ...
      await Post.findOneAndUpdate(
        { postId },
        {
          ...
          $inc: {
            likesCount: 1,
            dislikesCount: post.dislikes.some(
    ...
    if (alreadyDisliked) {
      ...
      await Post.findOneAndUpdate(
        { postId },
        {
          ...
          $inc: {
            dislikesCount: 1,
            likesCount: post.likes.some((id: any) => id.toString() === userId)
              ? -1
              : 0,
          },
        },
...
export const handleAdminGetAllPosts = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
...
export const handleAdminGetAllUsers = async (
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, data: users, totalCount: users.length });
  } catch (error) {
    if (error instanceof Error) {
      res.status(500).json({ success: false, message: error.message });
    } else {
...
export const handleAdminDeletePost = async (
  ...
  try {
    ...
    await User.updateMany(
      ...
      {
        $pull: {
          likedPosts: deletedPost._id,
          dislikedPosts: deletedPost._id,
          viewHistory: { postId: deletedPost._id },
        },
      },
    );
